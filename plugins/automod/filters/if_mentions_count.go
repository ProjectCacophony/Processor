...
func (f MentionsCount) NewItem(env *models.Env, args []string) (interfaces.FilterItemInterface, error) {
	...
	comparisionType, err := extractAmountComparisonType(args[0])
	if err != nil {
		return nil, err
	}
	...
	if amount < 1 {
		return nil, errors.New("amount has to be greater than 0")
	}
...
type MentionsCountItem struct {
	amount      int
	comparision AmountComparisonType
}
